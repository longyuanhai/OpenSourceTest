...
void Server::WaitForClient()
{
	...
	//	::InetNtop(addr_clt.sin_family, &addr_clt, buf_ip, IP_BUF_SIZE);
		cout << "A new client connected...IP address: " << ", port number: " << ::ntohs(addr_clt.sin_port) << endl;
...
DWORD WINAPI CreateClientThread(LPVOID lpParameter)
{
	...
	do
	{
		...
		if (ret_val > 0)
		{
			...
			snd_result = ::send(sock_clt, buf_msg, ret_val, 0);

			cout << "Message send: " << buf_msg << endl;
